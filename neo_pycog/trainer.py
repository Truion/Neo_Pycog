...
class Trainer:
    ...
    """
    def __init__(self, params, floatX=np.float64):
        """
        Initialize.
    ...
    def train(self, savefile, task, recover=True):
        ...
        if torchtools.get_processor_type()[1] == 'gpu':
        ...
        if Nin > 0:
            Win = torchtools.shared(Win_0,self.device,requires_grad=True)
        else:
            Win = None
        Wrec = torchtools.shared(Wrec_0,self.device,requires_grad=True)
        Wout = torchtools.shared(Wout_0,self.device,requires_grad=True)
        brec = torchtools.shared(brec_0,self.device,requires_grad=True)
        bout = torchtools.shared(bout_0,self.device,requires_grad=True)
        x0   = torchtools.shared( x0_0 ,self.device,requires_grad=True)
        ...
        if Nin > 0:
            if self.p['Cin'] is not None:
                ...
                Cin_mask_plastic = torchtools.shared(C.mask_plastic)
                Cin_mask_fixed   = torchtools.shared(C.mask_fixed)
        ...
        if self.p['Crec'] is not None:
            ...
            Crec_mask_plastic = torchtools.shared(C.mask_plastic)
            Crec_mask_fixed   = torchtools.shared(C.mask_fixed)
        ...
        if self.p['Cout'] is not None:
            ...
            Cout_mask_plastic = torchtools.shared(C.mask_plastic)
            Cout_mask_fixed   = torchtools.shared(C.mask_fixed)
        ...
        if self.p['ei'] is not None:
            ...
            if self.p['ei_positive_func'] == 'abs':
                ...
                make_positive = torchtools.rectify
            else:
            ...
            # E/I
            ei    = torchtools.shared(self.p['ei'], name='ei')
        ...
        # u   = T.tensor3('u')
        x0_ = torch.tensor(x0, device=self.device)
        
        if Nin > 0:
            def rnn(u_t, x_tm1, r_tm1, WinT, WrecT):
                x_t = ((1 - alpha)*x_tm1
                       + alpha*(torch.matmul(r_tm1, WrecT)        # Recurrent
                                + brec                     # Bias
                                + torch.matmul(u_t[:,:Nin], WinT) # Input
        ...
        def getxr(u, x0 , Win, Wrec):
            storage_x=[]
            storage_r=[]
            for i in u:
                if self.save_values[0]==None:
                    x0 = rnn(i, x0, f_hidden(x0), Win.T, Wrec.T)
                    storage_x.append(x0)
                    storage_r.append(f_hidden(x0))
                else:
                    x0 = rnn(i, x0, f_hidden(x0), Wrec.T)
                    storage_x.append(x0)
                    storage_r.append(f_hidden(x0))
            return storage_x, storage_r
        ...
        # z = f_output(torch.matmul(r, Wout_.T) + bout)
        def getz(r, Wout, bout):
            return f_output(torch.matmul(r, Wout.T) + bout)
        ...
        # loss = torch.sum(f_loss(z, target[:,:,:Nout])*mask)/masknorm
        def get_loss(z,target,mask,masknorm):
            return torch.sum(f_loss(z, target[:,:,:Nout])*mask)/masknorm
        # Root-mean-squared error
        def get_error(z,target,mask,masknorm):
            return torch.sqrt(torch.sum(torchtools.L2(z, target[:,:,:Nout])*mask)/masknorm)
        # error = torch.sqrt(torch.sum(torchtools.L2(z, target[:,:,:Nout])*mask)/masknorm)
        ...
        settings['(Numpy) floatX']   = self.floatX
        ...
        sgd = SGD(trainables, getxr, getz, get_loss, get_error, get_reg_w_L2, f_hidden, Nout , x0_, regs, self.p, save_values,rnn,
